		...
		case 'itemClicked':
			...
			switch (clickedBinding) {
				...
				case 'sgCtrlUserId':
					_onClickModal = function () {
						let p = {
							modalSgCtrlUserId: $('#managerSelect').val(),
							modalSgCtrlUserName: $("#managerSelect option:checked").text()
						};

						if(p.modalSgCtrlUserId == '0'){
							alert('담당자를 선택해 주세요.');
							return false;
						}

						let row = gridObj.getRowData(params.row);
						row.sgCtrlUserId = p.modalSgCtrlUserId;
						// row.sgCtrlUserName = p.modalSgCtrlUserName;
						gridObj.setRowData(params.row, row);
						$("#managerModal").modal('hide');
					};
					if($("#managerExist".val == '0')){
						setManagerModal();
					}
					$("#managerModal").modal();
					break;
...
function addRowData() {
	console.log('[itemmaster.js] addRowData', rowInit);
...
function setUuidModal(params){
	...
	$("#fileImageUuid").val(gridObj.getRowData(params.row).imageUuid);
...
function saveExcel() {
	gridObj.saveToExcel('item-master.xlsx', '표준상품관리');
}
...
function setManagerModal(){
	...
	$.ajax({
		type: "POST",
		url: "/opmanager/user/manager/findManagerList",
		beforeSend: function (xhr) {
		},
		success: function (data) {
			console.log(data);
			data.forEach(function(param,index){
				$('#managerSelect').append(
					'<option value="'+param.userId+'">'+param.userName+'</option>'
				);
			});
		}
	})
